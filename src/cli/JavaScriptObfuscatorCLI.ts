...
export class JavaScriptObfuscatorCLI implements IInitializable {
  ...
  private static processSourceCodeWithSourceMap(
    ...
    const obfuscatedCode: IObfuscatedCode = JavaScriptObfuscator.obfuscate(
      sourceCode,
      options
    );
  ...
  public run(): void {
    const canShowHelp: boolean =
      !this.arguments.length || this.arguments.includes("--help");
    ...
    const sourceCodeData: TSourceCodeData = new SourceCodeReader(
  ...
  private buildOptions(): TInputOptions {
    const inputCLIOptions: TInputOptions = JavaScriptObfuscatorCLI.filterOptions(
      this.inputCLIOptions
    );
    const configFilePath: string | undefined = this.inputCLIOptions.config;
    const configFileLocation: string = configFilePath
      ? path.resolve(configFilePath, ".")
      : "";
    const configFileOptions: TInputOptions = configFileLocation
      ? CLIUtils.getUserConfig(configFileLocation)
      : {};
  ...
  private configureCommands(): void {
    this.commands
      ...
      .version(process.env.VERSION || "unknown", "-v, --version")
      ...
      .option(
        ...
        "Disable rotation of unicode array values during obfuscation",
        BooleanSanitizer
      )
      .option(
        "--seed <number>",
  ...
  private configureHelp(): void {
    this.commands.on("--help", () => {
      ...
      console.log(
        "    %> javascript-obfuscator input_file_name.js --compact true --self-defending false"
      );
      console.log(
        "    %> javascript-obfuscator input_file_name.js --output output_file_name.js --compact true --self-defending false"
      );
      console.log(
        "    %> javascript-obfuscator input_directory_name --compact true --self-defending false"
      );
      console.log("");
    });
  }
  ...
  private processSourceCodeData(sourceCodeData: TSourceCodeData): void {
    ...
    if (!Array.isArray(sourceCodeData)) {
      const outputCodePath: string =
        outputPath || CLIUtils.getOutputCodePath(this.inputPath);

      this.processSourceCode(sourceCodeData, outputCodePath, null);
    } else {
      sourceCodeData.forEach(
        ({ filePath, content }: IFileData, index: number) => {
          console.log("ThongDN - " + filePath);
          var paths = filePath.split("/");
          paths = paths.slice(1);
          const newFilePath = paths.join("/");
          const outputCodePath: string = outputPath
            ? path.join(outputPath, newFilePath)
  ...
  private processSourceCode(
    ...
    if (sourceCodeIndex !== null) {
      const baseIdentifiersPrefix: string =
    ...
    if (options.sourceMap) {
      JavaScriptObfuscatorCLI.processSourceCodeWithSourceMap(
        sourceCode,
        outputCodePath,
        options
      );
    } else {
      JavaScriptObfuscatorCLI.processSourceCodeWithoutSourceMap(
        sourceCode,
        outputCodePath,
        options
      );
